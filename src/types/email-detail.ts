...
interface OrgEmailFormType
  extends Omit<OrgEmailDetailType, 'messenger' | 'skype'> {
  messenger: boolean;
  skype: boolean;
}
...
const emailValidation = Yup.array()
  .of(
    Yup.object().shape({
      ...
      email: Yup.string()
        .email('Valós email címet ajd meg')
  ...
  .test('UniqueEmail', 'Email típusa nem ismétlődhet', (values, ctx) => {
    ...
    const errors = extracted.map((item, idx) =>
      countMap.get(item) > 1
        ? ctx.createError({ path: `${ctx.path}.${idx}.email_type_id` })
        : false
    );
...
export { defaultEmailData, defaultOrgEmailData, emailValidation };
