...
interface PhoneBaseBoolType
  extends Omit<PhoneBaseType, 'messenger' | 'skype' | 'viber' | 'whatsapp'> {
  messenger: boolean;
  skype: boolean;
  viber: boolean;
  whatsapp: boolean;
...
interface PhoneFormType
  extends Omit<PhoneDetailType, 'messenger' | 'skype' | 'viber' | 'whatsapp'> {
  messenger: boolean;
  skype: boolean;
  viber: boolean;
  whatsapp: boolean;
...
const phoneValidation = Yup.array()
  .of(
    Yup.object().shape({
      phone_type_id: Yup.string().required(
        'Telefonszám típus kitöltése kötelező'
      ),
  ...
  .test('UniquePhone', 'Telefonszám típusa nem ismétlődhet', (values, ctx) => {
    ...
    const errors = extracted.map((item, idx) =>
      countMap.get(item) > 1
        ? ctx.createError({ path: `${ctx.path}.${idx}.phone_type_id` })
        : false
    );
...
export { defaultOrgPhoneData, defaultPhoneData, phoneValidation };
